    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group">
                ...
                <a href="{{ route('back.article.index') }}" class="btn bg-warning-light" data-toggle="tooltip" data-placement="top" title="Rafraîchir"><i class="fa fa-refresh"></i></a>
                ...
                <a href="{{ route('back.article.create') }}" class="font-weight-bold btn btn-primary btn-sm pull-right"><i class="fa fa-plus"></i> Ajouter un(e) Article</a>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-body">
                    <table id="table_article" class="table table-hover mb-0">
                        <thead>
                            <tr>
                                <th>
                                ...
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input style="margin-left: 0.5px; cursor: pointer" class="form-check-input checkbox-child" type="checkbox" value="{{ $article->id }}">
                                        </div>
                                    </td>
                                    <td data-id="{{ $article->id }}">#{{ $loop->index + 1 }}</td>
                                    <td>{{ $article->titre }}</td>
<td>{{ $article->contenu }}</td>
<td>{{ $article->categorie_id }}</td>
<td>{{ $article->responsable_id }}</td>

                                    <td>
                                        <a href="{{ route('back.article.show', $article->id) }}" class="btn bg-success-light btn-sm mr-1" title="Voir">
                                            <span class="text-success"><i class="fa fa-eye"></i></span>
                                        </a>
                                        <a href="{{ route('back.article.edit', $article->id) }}" class="btn bg-warning-light btn-sm mr-1" title="Modifier">
                                            <span class="text-warning"><i class="fa fa-pencil-square-o "></i></span>
                                        </a>
                                        <a href="{{ route('back.article.delete', $article->id) }}" class="bg-danger-light btn-delete btn-sm" title="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer ce ou cette article ?')">
    ...
    <script>
        ...
        $('.btn-delete-all').click(function(e) {
            if (confirm("Voulez-vous vraiment supprimer les catégories sélectionnées ?")) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('back.article.delete_all') }}",
