    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group">
                ...
                <a href="{{ route('back.commentaire.index') }}" class="btn bg-warning-light" data-toggle="tooltip" data-placement="top" title="Rafraîchir"><i class="fa fa-refresh"></i></a>
                ...
                <a href="{{ route('back.commentaire.create') }}" class="font-weight-bold btn btn-primary btn-sm pull-right"><i class="fa fa-plus"></i> Ajouter un(e) Commentaire</a>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-body">
                    <table id="table_commentaire" class="table table-hover mb-0">
                        <thead>
                            <tr>
                                <th>
                                ...
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input style="margin-left: 0.5px; cursor: pointer" class="form-check-input checkbox-child" type="checkbox" value="{{ $commentaire->id }}">
                                        </div>
                                    </td>
                                    <td data-id="{{ $commentaire->id }}">#{{ $loop->index + 1 }}</td>
                                    <td>{{ $commentaire->article_id }}</td>
<td>{{ $commentaire->membre_id }}</td>
<td>{{ $commentaire->contenu }}</td>

                                    <td>
                                        <a href="{{ route('back.commentaire.show', $commentaire->id) }}" class="btn bg-success-light btn-sm mr-1" title="Voir">
                                            <span class="text-success"><i class="fa fa-eye"></i></span>
                                        </a>
                                        <a href="{{ route('back.commentaire.edit', $commentaire->id) }}" class="btn bg-warning-light btn-sm mr-1" title="Modifier">
                                            <span class="text-warning"><i class="fa fa-pencil-square-o "></i></span>
                                        </a>
                                        <a href="{{ route('back.commentaire.delete', $commentaire->id) }}" class="bg-danger-light btn-delete btn-sm" title="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer ce ou cette commentaire ?')">
    ...
    <script>
        ...
        $('.btn-delete-all').click(function(e) {
            if (confirm("Voulez-vous vraiment supprimer les catégories sélectionnées ?")) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('back.commentaire.delete_all') }}",
