    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group">
                ...
                <a href="{{ route('back.tag.index') }}" class="btn bg-warning-light" data-toggle="tooltip" data-placement="top" title="Rafraîchir"><i class="fa fa-refresh"></i></a>
                ...
                <a href="{{ route('back.tag.create') }}" class="font-weight-bold btn btn-primary btn-sm pull-right"><i class="fa fa-plus"></i> Ajouter un(e) Tag</a>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-body">
                    <table id="table_tag" class="table table-hover mb-0">
                        <thead>
                            <tr>
                                <th>
                                ...
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input style="margin-left: 0.5px; cursor: pointer" class="form-check-input checkbox-child" type="checkbox" value="{{ $tag->id }}">
                                        </div>
                                    </td>
                                    <td data-id="{{ $tag->id }}">#{{ $loop->index + 1 }}</td>
                                    <td>{{ $tag->nom }}</td>

                                    <td>
                                        <a href="{{ route('back.tag.show', $tag->id) }}" class="btn bg-success-light btn-sm mr-1" title="Voir">
                                            <span class="text-success"><i class="fa fa-eye"></i></span>
                                        </a>
                                        <a href="{{ route('back.tag.edit', $tag->id) }}" class="btn bg-warning-light btn-sm mr-1" title="Modifier">
                                            <span class="text-warning"><i class="fa fa-pencil-square-o "></i></span>
                                        </a>
                                        <a href="{{ route('back.tag.delete', $tag->id) }}" class="bg-danger-light btn-delete btn-sm" title="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer ce ou cette tag ?')">
