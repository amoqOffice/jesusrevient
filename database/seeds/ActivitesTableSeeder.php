...
class ActivitesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($data as $youtube_video) {
            ...
            count($activite->eglises) ? $activite->eglises()->attach($youtube_video->eglises) : ''; // Eglise

            count($activite->tags) ? $activite->tags()->attach($youtube_video->tags) : ''; // Tag

            ($youtube_video->type) ? $youtube_video->type->activites()->save($activite) : ''; // Type

            count($activite->responsables) ? $activite->responsables()->attach($youtube_video->responsables) : ''; // Responsable

            count($activite->rubriques) ? $activite->rubriques()->attach($youtube_video->rubriques) : ''; // Rubrique
        }
    }
}
